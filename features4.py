        ...
        for reverse in [False, True]:
            for flip in [False, True]:
                if (is_forward(video) and (not reverse)) or ((not is_forward(video)) and reverse):
                    ...
                    direction = 'b'
                imgs = load_video(video, './data/ArrowDataAll/', flip = flip, reverse = reverse, mask = lambda x: x[:2] == 'of', grayscale = False)
                imgs = map(lambda x: cv2.resize(x, (227, 227)), imgs)
                imgs = map(lambda x: x - np.ones_like(x)*128, imgs)
                imgs = map(lambda x: np.moveaxis(x, -1, 0), imgs)
                imgs = np.asarray(imgs)
                flows = load_flows(video, direction)
                sel = np.asarray([[2*i, 2*i+1] for i in select(flows, 20)]).flatten()
                imgs = np.take(imgs, sel, axis = 0)
                for i in range(len(imgs)/2):
                    net.blobs['data'].data[...] = imgs[2*i:2*i+2]
                    output = net.forward()
                    ...
                    if direction == 'f':
                        ...
                        y = np.append(y, 0)
        with open(os.path.join('./data/ArrowDataAll', video, 'features4.csv'), 'w') as f:
            np.savetxt(f, X, delimiter = ',', fmt = '%f')
        with open(os.path.join('./data/ArrowDataAll', video, 'labels4.csv'), 'w') as f:
...
if __name__ == '__main__':
    ...
    caffe.set_mode_gpu()
    # load original model
    model_def = 'models/caffenet4/deploy.prototxt'
    model_weights = 'models/caffenet4/bvlc_reference_caffenet.caffemodel'
