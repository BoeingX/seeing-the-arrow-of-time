        ...
        for reverse in [False, True]:
            for flip in [False, True]:
                ...
                for i in range(len(flows) / 2):
                    imgs.append(np.stack([flows[2*i], flows[2*i+1]], axis = 0))
                imgs = np.asarray(imgs)
                net.blobs['data'].data[...] = imgs
                output = net.forward()
                X = np.append(X, output['fc7'], axis = 0)
                if direction == 'f':
                    y = np.append(y, np.ones(len(imgs)))
                else:
                    y = np.append(y, np.zeros(len(imgs)))
        with open(os.path.join('./data/ArrowDataAll', video, 'features1.csv'), 'w') as f:
            np.savetxt(f, X, delimiter = ',', fmt = '%f')
        with open(os.path.join('./data/ArrowDataAll', video, 'labels1.csv'), 'w') as f:
...
if __name__ == '__main__':
    ...
    model_def = 'models/caffenet1/deploy.prototxt'
    model_weights = 'models/caffenet1/bvlc_reference_caffenet.caffemodel'
    net = caffe.Net(model_def, model_weights, caffe.TEST)
    net.params['conv1-stack'][0].data[...] = conv1_stack

    # create transformer for the input called 'data'
    net.blobs['data'].reshape(20, 2, 227, 227)
