    ...
    for video in videos:
        ...
        for reverse in [False, True]:
            for flip in [False, True]:
                imgs_ = load_video(video, './data/ArrowDataAll/', flip = flip, reverse = reverse, mask = lambda x: x[:2] == 'im')
                ...
                idx = select(flows, 5)
                imgs_ = np.take(imgs_, idx, axis = 0)
                imgs_ = map(lambda x: transformer.preprocess('data', x), imgs_)
                imgs = []
                for i in range(len(imgs_) - 2):
                    imgs.append(np.concatenate(imgs_[i:i+3], axis = 0))
                imgs = np.asarray(imgs)
                for i in range(len(imgs) / 3):
                    net.blobs['data'].data[...] = imgs[3*i:3*(i+1)]
                    output = net.forward()
                    X = np.append(X, output['fc7'], axis = 0)
                if direction == 'f':
                    y = np.append(y, np.asarray(np.ones(len(imgs))))
                else:
                    y = np.append(y, np.asarray(np.zeros(len(imgs))))
        with open(os.path.join('./data/ArrowDataAll', video, 'features3-bis.csv'), 'w') as f:
            np.savetxt(f, X, delimiter = ',', fmt = '%f')
        with open(os.path.join('./data/ArrowDataAll', video, 'labels3-bis.csv'), 'w') as f:
...
if __name__ == '__main__':
    ...
    caffe.set_mode_gpu()
    # load original model
    model_def = 'models/caffenet3-bis/deploy.prototxt'
    model_weights = 'models/caffenet3-bis/bvlc_reference_caffenet.caffemodel'
    ...
    net.blobs['data'].reshape(3, 9, 227, 227)
